    ...
    /*public function showWelcome() {
        ...
        if ($user_slug && $user_type == 'Restaurant') {
            $caterer = DB::table('users')
                    ...
                    ->select("users.*", "opening_hours.catering_type", "opening_hours.average_time", "opening_hours.estimated_cost", "opening_hours.open_close", "opening_hours.open_days", "opening_hours.start_time", "opening_hours.end_time", "opening_hours.breakfast_time", "opening_hours.minimum_order", "areas.name as area_name", "cities.name as city_name", 'opening_hours.catering_type')
                    ->first();
            if ($caterer) {
                $restData = 1;
            }
            ...
            if ($url2[0] != 'www') {
                ...
                if(!empty($items)){
                     foreach($items as $k => $v){
                         $food_cat = DB::table('restaurant_categories')->where('id',$v->category_id)->first();
                         $items[$k]->cat_name = (isset($food_cat->cat_name) || !empty($food_cat->cat_name))?$food_cat->cat_name:'';
                      }
                     $collection = new Collection($items);
                     $sorted = $collection->sortBy('cat_name');
                     $sorted->values()->all();
...
if (empty($caterer)) {
            return Redirect::to(MAIN_HTTP_PATH);
                ...
                $food_cat =  DB::table('restaurant_categories')
                        ->orderBy('restaurant_categories.cat_name', 'asc')
                        ->where("menu_item.user_id", "=", $caterer->id)
                        ->join('menu_item', 'restaurant_categories.id', '=', 'menu_item.category_id')
                        ->select("restaurant_categories.cat_name", "restaurant_categories.id")
...
//                ->orderBy('users.id', 'desc')
    ...
    public function commingsoon()
     {
         return View::make('home.comming-soon');
     }
    ...
    public function showWelcome() {
        ...
        if ($restData == 1) {
            ...
            if ($url2[0] != 'www') {
                ...
                //get timezone and currency start
                ...
                $timezone = $timezone_currency->timezone;
                
                $ex = explode(",", $caterer->start_time);
                $starts = [];
                $start_time = [];
                foreach($ex as $arr)
                {
                    $ex2 = explode("to", $arr);
                    array_push($starts, $ex2[0]);
                }
                $start_time = implode(",", $starts);
                
                $ex = explode(",", $caterer->end_time);
                $ends = [];
                $end_time = [];
                foreach($ex as $arr)
                {
                    $ex2 = explode("to", $arr);
                    array_push($ends, $ex2[0]);
                }
                $end_time = implode(",", $ends);
                //get timezone and currency end
...
//            $query->orwhere(DB::raw("CONCAT_WS(' ', tbl_users.first_name,tbl_users.last_name)"), "like", "%$keyword%");
    ...
    function showMenu($slug) {
        ...
        $caterer = DB::table('users')
                ...
                ->leftjoin('areas', 'areas.id', '=', 'users.area')
                ->leftjoin('cities', 'cities.id', '=', 'users.city')
                ->select("users.*", "opening_hours.catering_type", "opening_hours.breakfast_time", "opening_hours.start_time", "opening_hours.end_time", "opening_hours.average_time", "opening_hours.estimated_cost", "opening_hours.open_close", "opening_hours.open_days", "opening_hours.start_time", "opening_hours.end_time", "opening_hours.minimum_order", "areas.name as area_name", "cities.name as city_name", 'opening_hours.catering_type')
                ->first();
    ...
    function showAddtocart($address_id = null) {
        ...
        if (!empty($input)) {
            ...
            ->first();
             ...
             //print_r($rest_user); 
            $tz = DB::table('timezone_currency')
                        ->where("timezone_currency.user_id", "=", $rest_user->id)
                        ->first();
        ...
        $content = $cart->contents(true);
        $view = View::make('home.cart')->with('cart', $cart)->with('currency', $currency)->with('cart_content', $content)->with('order', (isset($input['order']) ? $input['order'] : 0));
        $html = $view->render();
...
//        // get my addresses   
        ...
        if (!empty($input)) {
            ...
            if(isset($input['restaurant_id']) && !empty($input['restaurant_id']))
            {
                $tz = DB::table('timezone_currency')
                        ->where("timezone_currency.user_id", "=", $input['restaurant_id'])
                        ->first();    
            }
            else{
                $tz = DB::table('timezone_currency')
            ...
            $date = new DateTime(null, new DateTimeZone($tz->timezone));
            ...
            if ($validator->fails()) {
                ...
                if (!empty($order_id)) {
                    $orderData = array(
                        ...
                        'discount' => $discount,
                        'status' => "Pending",
                        'payment_method' => ($input['payment_method'] == "bank_transfer") ? 1 : 0,
                    );
                    DB::table('orders')
                            ->where('id', $order_id)
                            ->update($orderData);
                    ...
                    if (isset($input['pickup_ready']) && $input['pickup_ready'] == 1) {
                        ...
                        $orderData['pickup_now'] = $input['pickup_now'];
                        $orderData['pickup_time'] = $input['pickup_time'];
                        $orderData['pick_comment'] = $input['pick_comment'];
                ...
                if (!empty($content)) {
                    ...
                    foreach ($content as $cartData) {
                        ...
                        $data = array(
                            ...
                            'quantity' => $cartData['quantity'],
                            'comment' => $comment,
                            'submenus' => (isset($cartData['submenus']) && !empty($cartData['submenus'])) ? $cartData['submenus'] : NULL,
                            'order_id' => $order_id,
                            'sub_total' => $subtotal,
                            'user_id' => $user_id,
                            'addon_id' => $addons,
