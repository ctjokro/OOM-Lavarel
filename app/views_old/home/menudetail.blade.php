    ...
    <div class="platenumber">
        ...
        //  echo count($addonData);
          ...
          foreach($variantData as $addonDataVal){
              ...
              ?><li><label>{{$addonDataVal->name}}</label> <span class="pricev">{{$addonDataVal->price}}</span> <span data-name-variant="{{$addonDataVal->name}}" data-price-variant="{{$addonDataVal->price}}" data-id-variant="{{$addonDataVal->id}}" data-type="Variant" id="<?php echo $idNCV; ?>" class="btnv">Get it</span></li><?php
          }
          echo "</ul>";
          ?></div></div><?php
        }
