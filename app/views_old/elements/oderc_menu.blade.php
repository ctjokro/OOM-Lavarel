    ...
    <?php if ($userData->user_type == 'Customer' || $userData->user_type == 'Restaurant') { ?>
        <?php if (!empty($orderData)) { ?>
            <div class="reset_clsd">
                    ...
                    <?php foreach ($orderData as $odata) { ?>
                        <li>
                            <div class="rest_bx">
                                ...
                                <span>{{ html_entity_decode(HTML::link(HTTP_PATH.$viewLink.$odata->slug, "View Details", array('class' => 'orderviewlink'), true)) }}</span>
                                <?php } else { ?>
                                <span>{{ html_entity_decode(HTML::link(HTTP_PATH.$viewLink.$odata->slug, "View Details", array('class' => 'orderviewlink'), true)) }}</span>
                                <?php } ?>
                            </div>
                ...
                <div class="vi_all">
                    {{ html_entity_decode(HTML::link(HTTP_PATH.$viewAllLink, "View All", array('class' => 'orderviewlinkall'), true)) }}
                </div>
            </div>
        <?php } ?>
    <?php } ?>
